    ...
    <div>
      ...
      {analysis.status === 'PROCESSING' && (
        <div className="bg-yellow-50 border border-yellow-200 rounded p-4 mb-6">
          <p className="text-yellow-800">Analysis is still processing. Please wait...</p>
        </div>
      )}
      ...
      {analysis.status === 'COMPLETED' && analysis.classification_result && (
        <div className="bg-white rounded-lg shadow p-6">
          <ClassificationResultComponent result={analysis.classification_result} />
        </div>
      )}
    </div>
  );
